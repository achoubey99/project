...
extern bool regWrite(signUp*);
extern void regRead(vector<signUp>&);
int main()
{
                  ...
                  if(is_number(rcvDataBuf)){
                     ...
                     switch(atoi(rcvDataBuf))
                     {
                        case 1:
                              ...
                              //strcat(c, S->setDetails());
                              //strcat(c, " ------------");
                             /*
                              send(newClientFd, c, strlen(c), 0); 
                              memcpy(&rcvDataBuf,S->setDetails());

                              send(newClientFd, "registration successful", strlen("registration successful"), 0);

                              memset(&rcvDataBuf, 0, maxDataSize);

                              */
                              //cout<<"Msg Received: "<<S->setDetails()<<endl;
                        ...
                        case 2:
                              ...
                              {
                                 if(strcmp(it->getlId(),l->getlId())==0)
                                    count++;
                                 if(strcmp(it->getlPasswd(),l->getlPasswd())==0)
                                    count++;
                                 if(count == 2)
                                    {
                                       cout<<"true"<<endl;
